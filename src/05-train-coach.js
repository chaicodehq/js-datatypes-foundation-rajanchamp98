...
return passengers.find(p=>p.name.toLowerCase()==name.trim().toLowerCase())
...
return passengers.findIndex(p=>p.name.toLowerCase()== name.trim().toLowerCase())
...
//  *      - Example: areAllConfirmed([{status:"confirmed"}, {status:"confirmed"}]) => true
if(!Array.isArray(passengers) || passengers.length==0) return false
if(passengers.every(p=>p.status=='confirmed')) return true
return false

...
export function getChaiOrderLength(order) {
  ...
  const lengthOrder=trimmedOrder.length
  return lengthOrder
}
...
export function hasSpecialIngredient(order, ingredient) {
  ...
  const lowerCaseOfOrder=order.trim().toLowerCase()
  const lowerCaseOfIngredient=ingredient.trim().toLowerCase()
  if(lowerCaseOfOrder.includes(lowerCaseOfIngredient)){
    return true
  }
  return false
